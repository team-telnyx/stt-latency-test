...
Customer-runnable benchmark for Telnyx standalone STT. Streams a sample WAV to the Telnyx STT WebSocket and reports realistic latency for Deepgram nova-3 and flux side-by-side.
...
The output reports two side-by-side numbers per metric:
...
Both columns are reported side-by-side. Wall-clock is the honest "what you'll see" number. Service-only isolates engine performance.
